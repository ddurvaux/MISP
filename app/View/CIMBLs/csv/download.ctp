...
fputcsv($fp, array(
    ...
    'vuln_mgt', 'event_id', 'event_detect_time', 'event_threat_type', 'event_threat_level', 'event_targeted_domain', 'event_start_time', 'event_end_time'
    )
);
foreach($cimbl['Event'] as $event){
    foreach($event['Attribute'] as $row) {
        fputcsv($fp, array(
            ...
            $event['Targeted_domain'], $event['start_time'], $event['end_time']
            )
        );
    }
}
